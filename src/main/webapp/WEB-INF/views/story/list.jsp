...
<script>
  ...
  const initCategory = function () {
    CATEGORY_LIST.forEach(function (obj, idx) {
      ...
      $('.category_tab_area').append(
          `<button class="category-tab \${idx == 0?'active':''}" data-category="\${id}" onclick="searchTab(this)">\${nm}</button>`);
    });
  }
  ...
  const defaultList = function ({categoryId, keyword}, callback) {
    ...
    comm.paging.getList({
      ...
      for (let i = 0; i < data.list.length; i++) {
        ...
        const summary = obj.SUMMARY.length < 100 ? obj.SUMMARY : (obj.SUMMARY || '').substring(0,
            100) + ' ...'
