const COMMENT = function () {
  ...
  const renderList = function (list) {
    ...
    if (list && list.length > 0) {
      for (let listObj of list) {
        ...
        COMMENT_DOM.setDataSet(COMMENT_ELEMENT.button.getUpdate(commentElement),
            listObj);
        COMMENT_DOM.setDataSet(COMMENT_ELEMENT.button.getDelete(commentElement),
            listObj);
        COMMENT_DOM.setDataSet(
            COMMENT_ELEMENT.button.getDeclaration(commentElement), listObj);
        COMMENT_DOM.setDataSet(
            COMMENT_ELEMENT.button.getUpdateConfirm(commentElement), listObj);
  ...
  const updateComment = function (id, regId) {
    ...
    if (target.classList.contains("ing")) {
      ...
      commentModifyButton.addEventListener("click", function () {
        param.comment = COMMENT_DOM.replaceLineBreakWithBrReturnValue(
            COMMENT_ELEMENT.textArea.getUpdate(writeWrap));
  ...
  const insertComment = function (id, type, callback) {
    let commentInsertParam = {
      ...
      contentsType: type,
      refContentsId: "0",
      comment: COMMENT_DOM.replaceLineBreakWithBrReturnValue(
          COMMENT_ELEMENT.textArea.getInsert()),
    };
    ...
    REQUEST.send(commentInsertApiUrl, "POST", commentInsertParam,
        function (resp) {
          // 등록성공
          if (resp.code == '0000') {
            ...
            COMMENT_DOM.setCount(COMMENT_ELEMENT.area.getCount(),
                COMMENT_DOM.getCount(COMMENT_ELEMENT.area.getCount()) + 1);
            COMMENT_DOM.renderCount(COMMENT_ELEMENT.area.getCount())

            COMMENT_DOM.setDataSet(
                COMMENT_ELEMENT.button.getUpdate(listElement), resp.comment);
            COMMENT_DOM.setDataSet(
                COMMENT_ELEMENT.button.getDelete(listElement), resp.comment);
            COMMENT_DOM.setDataSet(
                COMMENT_ELEMENT.button.getDeclaration(listElement),
                resp.comment);
            COMMENT_DOM.setDataSet(
                COMMENT_ELEMENT.button.getUpdateConfirm(listElement),
                resp.comment);
  ...
  const addEventDelete = function (element, callback) {
    // 삭제 이벤트 적용
    COMMENT_ELEMENT.button.getDelete(element).addEventListener("click",
        function () {
          const obj = this;
          if (callback) {
  ...
  const addEventDeclaration = function (element) {
    COMMENT_ELEMENT.button.getDeclaration(element).style.display = 'none';

    // 신고하기 이벤트 적용
    COMMENT_ELEMENT.button.getDeclaration(element).addEventListener("click",
        function () {
          const obj = this;
          declaration(
              COMMENT_DOM.getDataSet(obj, "id"),
  ...
  return {
    ...
    disabled: function () {
      $("textarea", COMMENT_ELEMENT.area.getInsert()).attr("placeholder",
          "관리자가 댓글작성을 제한했습니다.");
