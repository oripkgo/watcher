...
public class BoardController {
  ...
  @RequestMapping(value = {"/notice/view",
      "/my-story/{memId}/notice/view"}, method = RequestMethod.GET)
  public ModelAndView noticeView(
    ...
    Map<String, Object> noticeCurrent = (Map<String, Object>) noticeInfo.get("current");

    noticeService.insertViewsCount(noticeParam);

    // 게시물 수정권한 여부 s
    mav.addObject("modifyAuthorityYn", "N");
    if (StringUtils.hasText(loginId) && Objects.equals(noticeCurrent.get("REG_ID"), loginId)) {
  ...
  public ModelAndView showNoticeEditPage(
    ...
    if (StringUtils.hasText(noticeParam.getId())) {
      noticeInfo = noticeService.getView(noticeParam);
      regId = noticeInfo.get("REG_ID").toString();
    }

    // 게시물 수정권한 여부 체크
    mav.addObject("modifyAuthorityYn", "N");
    if (StringUtils.hasText(loginId) && Objects.equals(regId, loginId)) {
      mav.addObject("modifyAuthorityYn", "Y");
    }
  ...
  public LinkedHashMap modifyBoardLike(
    ...
    try {
      ...
      if (sessionUtil.getSession(sessionId) != null) {
        loginId = sessionUtil.getSession(sessionId).get("LOGIN_ID");
      }
    } catch (Exception e) {
    }
  ...
  public LinkedHashMap insertComment(
    ...
    if (sessionUtil.getSession(sessionId) != null) {
      Map<String, String> userData = sessionUtil.getSession(sessionId);
    ...
    // 댓글 공백인지 입력여부 체크
    if (!param.containsKey("comment") || param.get("comment") == null || param.get("comment")
        .equals("")) {
